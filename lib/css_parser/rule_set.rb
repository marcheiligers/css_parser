module CssParser
  class RuleSet
    ...
    DIMENSIONS_PROPERTIES =  {
      ...
      'border-color' => DIRECTIONS.map { |dir| "border-#{dir}-color" },
    ...
    def create_dimensions_shorthand! # :nodoc:
      DIMENSIONS_PROPERTIES.each do |property, expanded|
        foldable = expanded.map { |name| @declarations[name] }.compact
        # All four dimensions must be present
        if foldable.length == 4
          values = foldable.map { |value| value[:value].downcase.strip }


          if values[3] == values[1] # left == right
            if values[0] == values[2] # top == bottom
              if values[0] == values[3] # top == left then All four sides are equal
                new_value = values[0] # top
              else # Top and bottom are equal, left and right are equal
                new_value = values[0] + ' ' + values[3] # top  left
              end
            else # Only left and right are equal
              new_value = values[0] + ' ' + values[0] + ' ' + values[2] # top  left  bottom
            end
          else # No sides are equal
            new_value = values.join(' ') # top  right  bottom  left
          end
          ...
          expanded.each { |name| @declarations.delete(name) }
        end
      end
    end
